...
declare namespace Q {
	...
	export interface Promise<T> {
		 ...
		 * In its static daftar, it is aliased as Q.try, since it has semantics similar to a try block (but handling both
	 ...
	 * This method is especially useful in its static daftar for wrapping functions to ensure that they are always
	 ...
	 * Returns a promise for the result of calling a function, with the given variadic arguments. Has the same return
	 * value/thrown exception translation as explained above for fbind.
	 * In its static daftar, it is aliased as Q.try, since it has semantics similar to a try block (but handling both synchronous
